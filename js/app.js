...
async function submitOrder() {
    ...
    await sb.from('orders').insert({
        ...
        comandas: document.getElementById('comandaName').value || '1',
...
async function checkCashStatus() {
    ...
    if (data) {
        ...
        document.getElementById('cashRegisterDot').style.background = '#2ecc71';
        ...
        cashOpen = false;
        cashSessionId = null;
        document.getElementById('cashRegisterDot').style.background = '#e63946';
...
function requireCashOpen() {
    if (!isManager() && !cashOpen) {
        showToast('Caixa fechado. Aguarde um gerente abrir o caixa.', 'error');
        return false;
    }
    return true;
}
